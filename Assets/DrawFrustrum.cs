...
public class DrawFrustrum : MonoBehaviour {
    
    public Camera thisCamera;
    private bool createdFrustrum = false;

    private void Update() {
        if(!createdFrustrum){
            GetComponent<MeshFilter>().mesh = CameraExtention.GenerateFrustumMesh(thisCamera);
            createdFrustrum = true;
        }
    }
